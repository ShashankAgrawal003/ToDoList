...
if (window.localStorage.getItem("goalsCompletedArray") == undefined) {
  ...
  window.localStorage.setItem(
    "goalsCompletedArray",
    JSON.stringify(goalsCompletedArray)
  );
}
...
class item {
  ...
  createItem(name, divContainer) {
    ...
    checkForCompleted.addEventListener("click", () =>
      this.complete(checkForCompleted, itemBox, input, arrayName)
    );
    ...
    edit.addEventListener("click", () =>
      this.edit(input, name, edit, arrayName)
    );
    ...
    remove.addEventListener("click", () =>
      this.remove(itemBox, name, arrayName)
    );
  ...
  edit(input, name, edit, arrayName) {
    if (input.disabled == true) {
      ...
      edit.innerHTML =
        '<i class="material-icons create" style="color:blue;">create</i>';
      window.addEventListener("keydown", (e) => {
        if (e.which == 13) {
          input.disabled = !input.disabled;
          edit.innerHTML =
            '<i class="material-icons create" style="color:grey;">create</i>';
          if (arrayName === "goalsTodos") {
            let indexof = goalsTodos.indexOf(name);
            goalsTodos[indexof] = input.value;
            window.localStorage.setItem(
              `${arrayName}`,
              JSON.stringify(goalsTodos)
            );
          } else {
  ...
  star(star, name, arrayName) {
    ...
    if (
      ...
      window.localStorage.setItem(
        "goalsStarArray",
        JSON.stringify(goalsStarArray)
      );
      star.innerHTML =
        '<i class="material-icons star_rate" style="color:#FFBA00;">star_rate</i>';
    } else {
